...
def updateNovelty(population, offspring, archive, k=15, add_strategy="random", _lambda=6, verbose=False):
    ...
    if (len(offspring) < _lambda):
        print("ERROR: updateNovelty, lambda(%d)<offspring size (%d)" %
              (_lambda, len(offspring)))
        return None
    ...
    if(add_strategy == "random"):
        ...
        if (verbose):
            ...
            for offs in soff[iLast:len(soff)]:
                print("    nov="+str(offs.novelty)+" fit=" +
                      str(offs.fitness.values)+" bd="+str(offs.bd))
    else:
        print("ERROR: updateNovelty: unknown add strategy(%s), valid alternatives are \"random\" and \"novel\"" % (
            add_strategy))
        return None
